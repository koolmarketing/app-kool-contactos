...
class ServiciosController extends Controller
{
	...
	public function SaveService(Request $data)
	{
		...
		//if($data->ajax()) {		
		// valor real = valor real del servicio
		//valor = el valor real de la sentencia JS valor - descuento (monto a cobrar)
		$SR              = new \App\Servicio;

		$SR->id_perfil          = $data->input('id');
		$SR->id_usuario         = Auth::id();
		$SR->estado             = 1;
		$SR->inicio             = $data->input('inicio');
		$SR->fin                = $data->input('fin');
		$SR->titulo             = $data->input('titulo');
		$SR->vendedor           = $data->input('vendedor');
		$SR->valor              = $data->input('valor_real');
		$SR->saldo              = $data->input('valor_real');
		$SR->serial      	    = $data->input('serial');
		$SR->titulo_retencion   = $data->input('retencion');
		$SR->iva 				= $data->input('iva');
		$SR->valor_retencion	= $data->input('calcular_retencion');
		$SR->valor_iva      	= $data->input('calcular_iva');
		$SR->costos         	= $data->input('costos');
		$SR->valor_real         = $data->input('valor');
		$SR->descuento          = $data->input('descuento');
